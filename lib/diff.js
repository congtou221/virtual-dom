...
diff.prototype.dfsWalk = function(oldNode, newNode, index, patches){
	...
	if(!newNode){
		...
		// update props
		var propPatches = this.diffProps(oldNode.props, newNode.props);
		if(propPatches){
...
diff.prototype.diffChildren = function(oldChildren, newChildren, index, currentPatches, patches){
	// reorder child node
	...
	_.each(oldChildren, function(oldChild, index){
		...
		this.dfsWalk(oldChild, newChild, currentNodeIndex, patches);
		leftNode = child;
	})
