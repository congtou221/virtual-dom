...
patch.prototype.patchWalk = function(node, walker, patches){
	...
	for(var i = 0 ; i < len ; i++){
		...
		walker.index++;
		this.patchWalk(child, walker, patches);
	}
	if(currentPatch){
...
patch.prototype.applyPatches = function(node, currentPatch){
	_.each(currentPatch, function(currentPatchItem, index){
		...
		switch (currentPatchItem.type){
			case 'TEXT':
				...
				break;
			case 'PROPS':
				this.setProps(node, currentPatchItem.props);
				break;
			case 'REORDER':
...
patch.prototype.setProps = function(node, prop){
